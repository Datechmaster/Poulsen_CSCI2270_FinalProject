...
/*
 ...
 Function Description:
 This method takes in two parameters and returns the shortest path between the two landmarks
 Example:
 Graph g;
 ...
 Postcondition: Display the integer for the shortest path between two landmarks and the string representation of the path taken
 */
void Graph::breadthFirstSearch(string landmark1, string landmark2)
{
    ...
    vertex *startingLandmark = nullptr;    //Vertex to store the starting landmark
    vertex *endingLandmark = nullptr;  //Vertex to store the ending vertex
    ...
    {
        cout << "One or more landmarks doesn't exist or they are the same landmark" << endl;
        return;
    }
    ...
    if (landmarks1 && landmarks2)
    {
        ...
        while (!(shortestPathQueue.empty()))
        {
            ...
            vertex *v = nullptr;   //Vertex to store the adjacent vertex
...
void Graph::depthFirstTraversal(string startLandmark)
{
    ...
    vertex *startingLandmark = nullptr;   //Vertex to store the starting vertex
...
void Graph::dijkstra(string startLandmark, string endLandmark)
{
    ...
    vertex *startingLandmark = nullptr;
    vertex temp;

    int minDistance = 0;

    //Loop through all of the vertices setting visited to false, previous to nullptr, set each vertex distance to 0
    for(int j = 0; j < vertices.size(); j++)
    {
        vertices[j].visited = false;
        vertices[j].previous = nullptr;
    ...
    startingLandmark->previous = nullptr;
    ...
    miles = minDistance;
    cout << "Cost to ship: $" << miles*.02 << endl;
    return;
}
...
/*
 ...
 some landmarks/edges may be deleted.
 Example:
 Graph g;
...
/*
 ...
 Function Description: This method will take a string for the name of a landmark, and pseudo-delete it from the graph structure
 Example:
 Graph g;
 g.deleteVertex("Golden Gate Bridge")
 Precondition: Vector class must be included. The vertex struct with corresponding variables must be initialized as well as the vertices vector. The adjacent vector/struct
 must be declared and initialized with corresponding variables. The deleteEdge method must be implemented.
 Postcondition: Will pseudo-delete the vertex representing the landmark in the graph structure.
 */

void Graph::deleteVertex(string landMark)
{
    vertex *v = nullptr;   //Vertex to store the landmark to be deleted
    ...
    //Delete all of the adj vertexes to the one being deleted
    v->adj.erase(v->adj.begin(), v->adj.end());

    //Delete any other edges connected to the vertex
    for (int i = 0; i < vertices.size(); i ++)
    {
        for (int k = 0; k < vertices[i].adj.size(); k++)
        {
            if (vertices[i].adj[k].v->name == landMark)
            {
